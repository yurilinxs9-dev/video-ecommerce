...
export default function StoreSettingsPage() {
  ...
  const set = <K extends keyof WidgetSettings>(
    key: K,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/api/settings?store=${slug}`, {
        method: 'PUT',
      ...
      <div className="max-w-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <section className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
            ...
            />
            <Toggle
              label="Paginação (dots)"
              value={settings.showDots}
              onChange={(v) => set('showDots', v)}
            />
            <Toggle
              label="Botão Curtir"
              value={settings.showLike}
              onChange={(v) => set('showLike', v)}
            />
            <Toggle
              label="Botão WhatsApp"
              value={settings.showWhatsapp}
              onChange={(v) => set('showWhatsapp', v)}
            />
            <Toggle
              label="Botão Compartilhar"
